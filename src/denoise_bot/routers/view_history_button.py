...
async def handle_view_history(
    ...
    try:
        ...
        for item in history_items:
            timestamp = datetime.datetime.fromisoformat(item["created_at"]).strftime(
                "%d.%m.%Y %H:%M"
            )
            button_text = f"{item['request'][:30]}... ({timestamp})"
            builder.add(
                InlineKeyboardButton(
                    text=button_text, callback_data=f"show_image:{item['image_url']}"
                )
            )
        builder.row(
            InlineKeyboardButton(
                text="🚪 Выйти из истории", callback_data="exit_history"
            )
        )

        builder.adjust(1)  # По одной кнопке в строке
        await message.answer(
            "Ваши предыдущие запросы:", reply_markup=builder.as_markup()
        )
...
async def handle_history_select(callback: CallbackQuery):
    ...
    image_file = BufferedInputFile(file=image_data, filename="downloaded_image.png")

    # 3. Отправляем изображение
    await callback.message.answer_photo(image_file, caption="Результат вашего запроса")
...
async def handle_exit_history(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        "Вы вернулись в главное меню:", reply_markup=None  # Удаляем инлайн-клавиатуру
    )
